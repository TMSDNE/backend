...
describe('AUTH ROUTER', () => {
  describe('POST ROUTE /LOGIN', () => {
    it('should return 200 on success', async () => {
      const res = await request(server).post(AUTH_API_URL + '/login').send(user);
    ...
    it('should return a message on fail', async () => {
      const res = await request(server).post(AUTH_API_URL + '/login').send(...user, password: '');
  ...
  describe('POST ROUTE /REGISTER', () => {
    it('should return 200 on success', async () => {
      const res = await request(server).post(AUTH_API_URL + '/register').send(user);
      expect(res.status).toEqual(200);
    });
    ...
    it('should return 400 on fail (not enough info to register an user)', async () => {
      const res = await request(server).post(AUTH_API_URL + '/register').send(...user, password: '');
